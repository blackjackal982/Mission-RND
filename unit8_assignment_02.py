...
def profile_timeit():
    ...
    for f in func_list:
        for i in range(1,5):
            timer = timeit.repeat(stmt='f('+str(10**i)+')',setup='from unit8_conversion_methods import '+ f.__name__+' as f',repeat=5,number = 1)
            minimum = "%.6f"%min(timer)
            t = ["%.6f" % j for j in timer]
            print(f.__name__ + ", count = " + str(10**i) + ", min = " + str(minimum) + ", actuals = " + str([j for j in t]))
