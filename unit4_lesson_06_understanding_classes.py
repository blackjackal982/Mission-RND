...
class Person:
    ...
    def get_bmi_result(self):
        height_in_cm = float(self.height)/3.28
        bmi_index = float(self.weight) /(height_in_cm)**2
        if (bmi_index < 18.5):
            return "underweight"
        elif (bmi_index >= 18.5 and bmi_index<23):
            return "healthy"
        elif (bmi_index >= 23):
            return "obese"
...
-using .__class__ and .__init__ methods
-saving states of instances
