...
def test_for():
    ...
    assert [] == result  #what happened here?
                         #iter was using iter.__next__ to obtain next elements in
                         #the above for so now it's having no element left and
                         #iteration is stopped by StopIteration
    result = []
    for x in l:
        result.append(x)
    assert [1,2,3] == result
...
-working of iterator
-working of __next__() in iterables
-working of list and tuple as iterators
"""