...
def test_generator_zip_student():
    assert [(1, 1, 5, 2)] == list(generator_zip([1],[1,2,3],[5,6,7,8],[2,3,9,0,6]))
    assert [] == list(generator_zip([1, 4], "abc", [1, 2, 3, 5], ""))
    assert [(1, 'a', 1, 'b'), (4, 'b', 2, 'c')] == list(generator_zip([1,4],"abc",[1,2,3,5],"bcn"))
    assert [] == list(generator_zip([1,4],""))
    assert [(1, 'a'), (4, 'b')] == list(generator_zip([1,4],"abc"))
