...
def test_equality():
    ...
    assert True == (0 == False) # what happened here?// 0 is binary for false hence both are equal
    assert True == (1 == True)
    assert False == ("a" == True)
...
-non empty variables can not be compared to bool values
-if condition can be satisfied by non empty data
-Nonetype is treated as false
"""
